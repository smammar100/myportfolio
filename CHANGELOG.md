...
- Nav-toolbar hover tooltips (`.toolbar__button-tip`) enlarged to match the contribution-graph tooltip exactly — `font-size` `0.625rem`→`0.75rem` (10→12px), `line-height: 1rem`, `font-weight: 500`, `border-radius: var(--radius-md)` — so all tooltips share one size.
- **Typography unified to one scale across every page.** Page H1 is now `text-3xl sm:text-4xl tracking-tight` everywhere (was `text-2xl` on /about, `text-3xl` on /articles, `text-4xl/5xl` on project pages); project-page section `<h2>` dropped from `text-2xl` to `text-xl` to match the shared `SectionHeading`; arbitrary sizes removed — body `text-[15px]` → `text-base`, contribution-graph axis + tunes artist `text-[11px]` → `text-xs` (12px, legible and aligned with the legend/badges). Scale: H1 `text-3xl sm:text-4xl` · H2 `text-xl` · H3 `text-lg` · card title / body `text-base` · secondary / meta `text-sm` · micro/axis `text-xs`.
...
- Contribution graph now spans the **whole calendar year** (`contributionsCollection(from:$from, to:$to)`, Jan 1 → Dec 31 of the current year) instead of the trailing 365 days, so activity is distributed across a Jan…Dec grid instead of bunched at the right edge. Year is part of the `unstable_cache` key (busts at the year boundary). Footer caption changed from "in the last year" → "in {year}".
- Contribution graph gained GitHub-style **month labels** (Jan…Dec, aligned to the week each month starts) and **weekday labels** (Mon/Wed/Fri) via a single CSS grid with an `auto` gutter column + `auto` header row for pixel-aligned placement.
- Contribution-day tooltip rewritten from the native `title` attribute (which browsers suppress while the site's custom cursor is hidden via `cursor: none`) to a CSS-only `:hover`/`focus` tooltip: inverted-surface monochrome (black-on-light / white-on-dark, like the project TOC popover), 0.1s ease-in fade with a 300ms hover-intent delay, downward caret, GitHub-style ordinal text ("10 contributions on May 11th."), keyboard-accessible with per-cell `aria-label`.
